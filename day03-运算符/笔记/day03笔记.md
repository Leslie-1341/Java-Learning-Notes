...
2.小数直接参与运算，得到的结果有可能是不精确的（与小数在计算机中的存储方式有关）
案例：
...
```java
package com.itheima.test;//为什么要导包
...
* <u>byte、short、char三种类型的数据在运算的时候，都会直接先提升为int，然后再进行运算。</u>
...
​	因为b1和b2都是byte类型的。所以在参与计算的时候，变量b1和变量b2里面的值都会自动提升为int类型的。<u>最终其实就是两个int类型的相加，最终结果也是int类型的。</u>
...
public class OperatorDemo2 {
    public static void main(String[] args) {
        double a = 12.3;
        int b = (int) a;//先把double型的a强制转换为int型，再赋值给b
...
* <u>当+操作中出现字符串时，此时就是字符串的连接符，会将前后的数据进行拼接，并产生一个新的字符串。</u>
...
解释：当字符串跟变量相加的时候，实际上是跟<u>变量里面的值</u>进行拼接。

# 8.字符的+操作

### 规则：

​	char类型的数据在进行运算时，会直接先提升为int，再进行运算

​	当+操作中出现了字符，会拿着字符到计算机内置的ASCII码表中去查对应的数字，然后再进行计算。

注意：有字符串的+是拼接操作，有字符的+将字符转换为ASCII码值查到对应的数字进行计算。
...
* / 和 % 的区别：两个都是做除法运算，/取结果的商。% 取结果的余数。
* 整数操作只能得到整数，如果想要得到小数，必须有浮点数参与运算。
* 字符串加法是拼接，字符加法找到ASCII码表计算
...
* 放在变量的前面，我们叫做先++。 比如：++a（先加后用）
* 放在变量的后面，我们叫做后++。 比如：a++（先用后加）
...
public class Test{
    ...
    System.out.println("y:" + y);//y:10，先把x的值赋值给y，x再自增
    System.out.println("z:" + z);//z:11，x先进行自增，再把x的值赋值给z
    }
}
```
...
+=：把左右变量值相加，再把结果赋值给左边的变量

![image-20260324220120525](C:\Users\61924\AppData\Roaming\Typora\typora-user-images\image-20260324220120525.png)
...
​	运算结果跟&是一模一样的，只不过具有短路效果。左边false，则右边不会再被执行。

### ||：

​	运算结果跟|是一模一样的。只不过具有短路效果。左边true，则右边不会再被执行。
...
​	如果用户名输入正确了，那么我们再判断密码是否正确，符合业务逻辑。

​	但是如果用户名输入错误了，那么现在还有必要去比较密码吗？不要了。
...
又叫做：三元表达式或者问号冒号表达式。根据判断结果得到不同内容。
...
* 计算关系表达式的值。
* 如果关系表达式的值为真，那么执行表达式1；关系表达式的值为假，那么执行表达式2。
...
![image-20260324230922296](C:\Users\61924\AppData\Roaming\Typora\typora-user-images\image-20260324230922296.png)

只要知道其中一点：

​	小括号优先于所有。

​	书写代码勤加小括号，提高代码可读性，避免由于优先级出错的问题。
