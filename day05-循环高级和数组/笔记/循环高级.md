...
​	不能单独存在。可以用在switch和循环中，表示结束/跳出整个循环。
...
public class LoopTest1 {
    public static void main(String[] args) {
        ...
        //1.得到1~100之间的每一个数字
        for (int i = 1; i <= 100; i++) {
        //2.判断每一个数字，符合规则，就打印过;不符合规则就打印真实数字
...
public class LoopTest2 {
    public static void main(String[] args) {
        ...
        //注意：可以用number的平方根作为上限简化循环（一个不是质数的数字一定有两个因子，分别小于和大于这个数的平方根）
...
1. 导包：`import java.util.Random;`(IDEA中会自动导包)
2. 创建对象:`Random r = new Random;`
3. 生成随机数:`int num =r.nextInt(bound 100);//随机数范围默认从0开始，包左不包右，此处表示0-99.`

![image-20260326153020332](C:\Users\61924\AppData\Roaming\Typora\typora-user-images\image-20260326153020332.png)
...
public class LoopTest2 {
    public static void main(String[] args) {
        ...
        //2.使用键盘录入去猜出这个数字是多少？
        //注意生成随机数的代码不能放在循环里面，否则每次都会生成一个新的随机数
        Scanner sc = new Scanner(System.in);
        while(true){
            System.out.println("请输入一个整数");
