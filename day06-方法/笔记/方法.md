...
​	方法（method）是程序中最小的执行单元（方法中的程序要么全都执行，要么全都不执行）

​	重复的/具有独立功能的代码可以抽取到方法中，需要用的时候直接调用。提高代码的复用性和可维护性。

* 注意：
  * 方法必须先定义后才可以使用
  * 方法创建后并不是直接可以运行的，需要在main方法中调用该方法后才执行
...
方法定义技巧：要干的事情是方法体；需要用到的变量是形参。

## 3. 带参数方法定义和调用

### 3.1 带参数方法定义和调用

<u>针对变量不确定的情况，直接将变量定义在小括号中，只定义不赋值；调用该方法时，只赋值不定义；注意定义和调用时括号里的参数个数必须对应</u>

* 定义格式：

  参数：由数据类型和变量名组成 -  数据类型 变量名

  参数范例：单个参数&多个参数（参数之间使用逗号分隔）
...
注意方法在调用的时候，形参和实参必须一一对应，否则程序将报错。
...
使用情况：在调用处要根据方法的结果，编写另一段代码，进行下一步操作
...
* 定义格式
  ...
  public static 返回值类型 方法名 (参数) { 
      方法体；
  	return 返回值;//没有返回值可以省略不写，上面返回值类型写void
  }
  ```
...
* return关键字：
  ...
  1. return语句下面不能跟代码，因为永远执行不到，属于无效代码
  2. 方法没有返回值，可以省略不写。如果书写，表明结束方法。
  3. 方法有返回值，必须要写。表示结束方法和返回结果。
...
* 在同一个类中，定义了多个同名的方法，这些方法具有同种功能
  ...
  * 多个方法的参数不相同，类型不同或者数量不同
  ...
  简记：<u>同一个类中，方法名相同、参数不同的方法，与返回值无关</u>（返回值类型不同不影响）
...
方法被调用之后就会进栈执行，栈按照后进先出的顺序依次执行被调用的方法，直至程序入口main出栈，代码执行完毕。

基本数据类型：变量中存储的是真实的数据（存储在栈内存中）

特点：赋值给其他变量，赋的是真实的值

引用数据类型：变量中存储的是地址值

​	new出来的都是，在栈内存中存的不是真实的数据，而是堆内存的地址值
...
- 传递基本数据类型时，传递的是真实的数据，形参的改变，不影响实际参数的值；在方法中设置返回值，才会覆盖main方法中实参的值
