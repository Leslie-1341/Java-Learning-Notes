...
![image-20260329124039026](C:\Users\61924\AppData\Roaming\Typora\typora-user-images\image-20260329124039026.png)
...
public class Test4 {
    public static void main(String[] args) {
        ...
        //4.新数组中已经存满元素了，依次打印出新数组元素验证复制成功
...
public class Test6 {
    public static void main(String[] args) {
        //1.把整数的每一位放到数组当中（针对需要对数字进行批量操作【反转】的情况）
        int[] arr = {1, 9, 8, 3};
        //2.加密
        //每位数加上5
        for (int i = 0; i < arr.length; i++) {
...
public class Test9 {
    public static void main(String[] args) {
        ...
        //2.定义新数组用于存储抽奖的结果，对于每次随机的结果，比较是否已经出现过，以此避免奖项重复
        int[] newArr = new int[arr.length];
        //3.抽奖
        Random r = new Random();
        //因为有5个奖项，所以这里要循环5次
...
//每一次都要跟新数组中进行比较，出现重复的情况需要重新抽取，效率较低
```
...
public class Test10 {
    public static void main(String[] args) {
        //1.把奖池里面的所有奖项打乱顺序，遍历奖池，即可实现随机抽取且不重复的目的
        int[] arr = {2, 588, 888, 1000, 10000};
        Random r = new Random();
        for (int i = 0; i < arr.length; i++) {
            //获取随机索引
