...
基本：数据值存储在自己的空间中；赋值给其他变量赋的是真实的值

引用：数据值存储在其他空间中，自己空间存储的是地址值；赋值给其他变量赋的是地址值，其他变量修改了地址中的数据，对应原变量的值也会改变
